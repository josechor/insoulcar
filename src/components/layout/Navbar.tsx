    ...
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-neutral-100 shadow-sm">
      ...
      >
        <div className="flex items-center justify-between h-16">
          ...
          <ul className="hidden md:flex items-center gap-1">
            {NAV_LINKS.map((link) => (
              <li key={link.href}>
                <Link
                  href={link.href}
                  className="px-4 py-2 text-sm font-medium text-neutral-600 hover:text-brand-700 hover:bg-brand-50 rounded-lg transition-colors"
                >
                  {link.label}
                </Link>
              </li>
          ...
          {/* Mobile menu button */}
          <button
            className="md:hidden p-2 rounded-lg text-neutral-600 hover:text-brand-700 hover:bg-neutral-100"
          ...
          <div className="md:hidden pb-4 border-t border-neutral-100 mt-2 pt-4">
              ...
              {NAV_LINKS.map((link) => (
                <li key={link.href}>
                  <Link
                    ...
                    className="block px-4 py-2.5 text-sm font-medium text-neutral-700 hover:text-brand-700 hover:bg-brand-50 rounded-lg transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
            ...
            <div className="flex gap-3 px-4">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 flex items-center justify-center gap-2 bg-green-500 text-white text-sm font-semibold px-4 py-2.5 rounded-lg"
              >
                <WhatsAppIcon />
                WhatsApp
              </a>
              <a
                href={`tel:${BUSINESS.phone}`}
                className="flex-1 flex items-center justify-center gap-2 bg-brand-700 text-white text-sm font-semibold px-4 py-2.5 rounded-lg"
              >
                <PhoneIcon />
                Llamar
              </a>
